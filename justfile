...
dependency-count:
  ...
  jq '.packages[] | select((.sourceInfo // "") | contains("rust cargo") | not) | .name' -r /tmp/meilisearch-nix-sbom.spdx.json | sort --unique | wc -l
  @echo 'Evaluating the traditional built SBOM...'
  jq '.packages[] | select((.sourceInfo // "") | contains("rust cargo") | not) | .name' -r /tmp/meilisearch-traditional-sbom.spdx.json | sort --unique | wc -l
